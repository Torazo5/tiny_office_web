...
import { getProfileStats, getUserProfile } from "@/lib/profile-data";
...
export default async function ProfilePage() {
  ...
  const [stats, profile] = await Promise.all([
    getProfileStats(user.id, performances),
  ...
  return (
    <>
      <Header user={user} />
      <main className="mx-auto flex w-full max-w-[1180px] flex-col gap-9 p-4 sm:p-8">
        <section className="flex flex-wrap items-end justify-between gap-5 border-b border-border pb-7">
          <div className="flex items-center gap-4">
            <Avatar size="xl" className="border border-border bg-secondary">
              <AvatarImage src={profile.avatarUrl} alt={`${profile.displayName} profile picture`} />
              <AvatarFallback>{profile.displayName.slice(0, 1).toUpperCase()}</AvatarFallback>
            </Avatar>
            <div>
              <h1 className="text-[28px] font-bold tracking-tight text-foreground">{profile.displayName}</h1>
              <p className="mt-1 font-mono text-[12px] text-muted-foreground">@{profile.tag}</p>
        ...
        <ProfileSettings displayName={profile.displayName} tag={profile.tag} />
