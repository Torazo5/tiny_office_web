...
export function YouTubeRequestButton({
  query,
  videoId,
  title,
  channelTitle,
  ...
  if (state?.success) {
    return <p className="text-[12px] font-medium text-success" role="status">Requested ✓</p>;
  }
  ...
  return (
    <form action={action} className="flex flex-col items-start gap-2">
      ...
      <input type="hidden" name="youtube_channel_title" value={channelTitle} />
      <input type="hidden" name="youtube_published_at" value={publishedAt ?? ""} />
      <input type="hidden" name="youtube_thumbnail_url" value={thumbnailUrl ?? ""} />
      <input type="hidden" name="source_path" value={sourcePath} />
      <div className="absolute -left-[10000px] h-px w-px overflow-hidden" aria-hidden="true">
        <label htmlFor={`youtube-request-website-${videoId}`}>Website</label>
        <input id={`youtube-request-website-${videoId}`} name="website" type="text" tabIndex={-1} autoComplete="off" />
      </div>
      <button
        type="submit"
        disabled={pending}
        className="rounded-lg bg-primary px-3.5 py-2 text-[12.5px] font-semibold text-primary-foreground transition-opacity hover:opacity-90 disabled:cursor-wait disabled:opacity-60"
      >
        {pending ? "Saving…" : "Request this video"}
      </button>
      {state?.error && <p className="text-[11.5px] text-primary" role="alert">{state.error}</p>}
    </form>
  );
}
